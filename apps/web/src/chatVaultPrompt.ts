...
export function buildVaultWorkspaceChatMessage(message: string, options: VaultWorkspaceChatMessageOptions): string {
  ...
  return [
    ...
    "For Xiaohongshu posts, follow `<profile>/xiaohongshu/sop/publish-note-with-cover.md` when present: text drafting and cover production are separate steps; before dry-run or publish, scan `assets/<YYYY-MM-DD-topic-slug>/cover.png` and stop with `BLOCKED: cover_asset_missing` if it is absent.",
    "When the user asks for cover/image work, prefer `baoyu-infographic` (`baoyu-infrograph` means the same skill) and save the final cover to `assets/<YYYY-MM-DD-topic-slug>/cover.png`.",
    "Internal notes such as lessons, metrics, evidence, strategy, or growth-signal analysis do not require cover images unless the user asks to convert them into a publishable Xiaohongshu post.",
