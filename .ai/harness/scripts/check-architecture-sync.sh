...
usage() {
  cat <<'USAGE_EOF'
Usage: .ai/harness/scripts/check-architecture-sync.sh [--mode off|advisory|strict] [--target <branch>] [--changed-files <file>] [--format text|json]
...
if [[ ! -x ".ai/harness/scripts/architecture-queue.sh" ]]; then
  if [[ "$mode" == "strict" ]]; then
    echo "[ArchitectureSync] strict gate failed: missing .ai/harness/scripts/architecture-queue.sh" >&2
    exit 1
  fi
  echo "[ArchitectureSync] WARN: missing .ai/harness/scripts/architecture-queue.sh; skipping advisory freshness gate" >&2
  exit 0
fi

if ! bash .ai/harness/scripts/architecture-queue.sh reindex --check >/dev/null; then
  echo "[ArchitectureSync] architecture request index is stale; run bash .ai/harness/scripts/architecture-queue.sh reindex" >&2
  exit 1
fi
...
if ! command -v bun >/dev/null 2>&1 || [[ ! -f ".ai/harness/scripts/capability-resolver.ts" ]]; then
  if [[ "$mode" == "strict" ]]; then
    echo "[ArchitectureSync] strict gate failed: missing bun or .ai/harness/scripts/capability-resolver.ts" >&2
    exit 1
  fi
  echo "[ArchitectureSync] WARN: missing bun or .ai/harness/scripts/capability-resolver.ts; skipping advisory freshness gate" >&2
  exit 0
fi
...
matches_json="$(printf '%s\n' "$changed_files" | bun .ai/harness/scripts/capability-resolver.ts match --paths-from - --format json)"
capabilities="$(
