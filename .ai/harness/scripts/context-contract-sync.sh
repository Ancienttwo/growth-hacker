...
usage() {
  cat <<'USAGE_EOF'
Usage:
  .ai/harness/scripts/context-contract-sync.sh sync-latest
  .ai/harness/scripts/context-contract-sync.sh sync-event --json '<event-json>'
USAGE_EOF
}
...
if [[ ( -z "$capability_id" || "$capability_id" == "root" ) && -n "$file_path" && -f ".ai/harness/scripts/capability-resolver.ts" ]] && command -v bun >/dev/null 2>&1; then
  resolver_json="$(bun .ai/harness/scripts/capability-resolver.ts match --path "$file_path" --format json 2>/dev/null || true)"
