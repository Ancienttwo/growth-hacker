...
contract_command="bash .ai/harness/scripts/verify-contract.sh --contract $contract_file --strict --report-file <temp>"
set +e
contract_output="$(bash .ai/harness/scripts/verify-contract.sh --contract "$contract_file" --strict --report-file "$contract_report" 2>&1)"
contract_exit=$?
set -e
...
if command -v jq >/dev/null 2>&1 && jq -e . "$contract_report" >/dev/null 2>&1; then
  jq -n \
    ...
    --arg command "bash .ai/harness/scripts/verify-sprint.sh" \
  ...
  cat > "$checks_report" <<EOF_CHECKS
{
  ...
  "command": "bash .ai/harness/scripts/verify-sprint.sh",
  "exit_code": $exit_code,
