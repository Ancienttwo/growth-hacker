...
usage() {
  cat <<'USAGE_EOF'
Usage:
  .ai/harness/scripts/capture-plan.sh --slug <slug> [--title <title>] [--status Draft|Approved]
...
if [[ "$execute" -eq 1 ]]; then
  [[ -f ".ai/harness/scripts/plan-to-todo.sh" ]] || { echo "Missing .ai/harness/scripts/plan-to-todo.sh" >&2; exit 1; }
  bash ".ai/harness/scripts/plan-to-todo.sh" --plan "$plan_file"
fi
