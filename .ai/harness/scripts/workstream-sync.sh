...
usage() {
  cat <<'USAGE_EOF'
Usage:
  .ai/harness/scripts/workstream-sync.sh ensure --block <capability-prefix> [--slug <slug>] [--title <title>] [--plan <plan-file>] [--slice <todo-id>] [--request <architecture-request>]
USAGE_EOF
}
...
if [[ ! -f ".ai/harness/scripts/capability-resolver.ts" ]] || ! command -v bun >/dev/null 2>&1; then
  echo "workstream-sync: .ai/harness/scripts/capability-resolver.ts and bun are required" >&2
  exit 2
fi

if ! resolver_json="$(bun .ai/harness/scripts/capability-resolver.ts match --path "$functional_block" --format json 2>&1)"; then
...
if [[ -x ".ai/harness/scripts/context-contract-sync.sh" ]]; then
  bash ".ai/harness/scripts/context-contract-sync.sh" sync-event --json "$event_json"
fi
