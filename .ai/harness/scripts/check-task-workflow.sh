...
usage() {
  cat <<'USAGE_EOF'
Usage: .ai/harness/scripts/check-task-workflow.sh [--strict]
USAGE_EOF
}
...
check_required_file() {
  ...
  if [[ "$path" == .ai/harness/scripts/* ]]; then
    ...
    if [[ -f "assets/templates/helpers/$helper_name" && -f ".ai/harness/scripts/$helper_name" ]]; then
      return 0
    fi
  fi
...
if [[ -f ".ai/harness/scripts/check-deploy-sql-order.sh" ]]; then
  if ! bash ".ai/harness/scripts/check-deploy-sql-order.sh" --quiet; then
    report_issue "Deploy SQL order check failed."
  fi
fi

if [[ -f ".ai/harness/scripts/check-brain-manifest.sh" ]]; then
  if ! bash ".ai/harness/scripts/check-brain-manifest.sh"; then
    report_issue "Brain manifest check failed."
  fi
fi

if [[ -f ".ai/harness/scripts/sync-brain-docs.sh" ]]; then
  if ! bash ".ai/harness/scripts/sync-brain-docs.sh" --check; then
