...
usage() {
  cat <<'USAGE_EOF'
Usage: .ai/harness/scripts/prepare-codex-handoff.sh [--reason <reason>] [--print-prompt]
USAGE_EOF
}
...
if [[ -f ".ai/harness/scripts/prepare-handoff.sh" ]]; then
  REPO_HARNESS_SKIP_RESUME_REFRESH=1 bash .ai/harness/scripts/prepare-handoff.sh "$reason"
fi

resume_args=(.ai/harness/scripts/codex-handoff-resume.sh --cwd "$repo" --reason "$reason")
if [[ "$print_prompt" -eq 1 ]]; then
  resume_args+=(--print-prompt)
fi

resume_output=""
if [[ -f ".ai/harness/scripts/codex-handoff-resume.sh" ]]; then
