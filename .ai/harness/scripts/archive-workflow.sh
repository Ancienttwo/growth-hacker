...
usage() {
  cat <<'USAGE_EOF'
Usage: .ai/harness/scripts/archive-workflow.sh --plan <plan-file> --outcome <Completed|Abandoned|Superseded>
USAGE_EOF
}
...
if [[ -x ".ai/harness/scripts/refresh-current-status.sh" ]]; then
  bash ".ai/harness/scripts/refresh-current-status.sh" --clear --write --reason "archive-workflow" || true
fi
