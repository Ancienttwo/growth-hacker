...
usage() {
  cat <<'USAGE_EOF'
Usage:
  .ai/harness/scripts/contract-worktree.sh start --plan <plan-file> [--path <worktree-path>] [--branch <branch-name>]
  .ai/harness/scripts/contract-worktree.sh finish [--merge|--no-merge] [--target <branch>] [--message <commit-message>]
  .ai/harness/scripts/contract-worktree.sh cleanup --slug <slug> [--target <branch>] [--dry-run]
  .ai/harness/scripts/contract-worktree.sh status
USAGE_EOF
}
...
check_architecture_freshness() {
  ...
  if [[ -x ".ai/harness/scripts/check-architecture-sync.sh" ]]; then
    bash ".ai/harness/scripts/check-architecture-sync.sh" --target "$target_branch"
    return $?
  fi

  mode="$(policy_get '.architecture.freshness_gate' 'advisory')"
  if [[ "$mode" == "strict" ]]; then
    echo "contract-worktree: strict architecture freshness gate failed: missing .ai/harness/scripts/check-architecture-sync.sh" >&2
    return 1
  fi

  echo "contract-worktree: WARN missing .ai/harness/scripts/check-architecture-sync.sh; skipping advisory architecture freshness gate" >&2
  return 0
}
...
start_worktree() {
  ...
  (
    ...
    if [[ "$run_plan_to_todo" -eq 1 && -f ".ai/harness/scripts/plan-to-todo.sh" ]]; then
      REPO_HARNESS_CONTRACT_WORKTREE=1 bash ".ai/harness/scripts/plan-to-todo.sh" --plan "$plan_file"
    fi
  )
...
archive_finished_workflow() {
  ...
  [[ -x ".ai/harness/scripts/archive-workflow.sh" ]] || { echo "contract-worktree: .ai/harness/scripts/archive-workflow.sh is missing or not executable" >&2; exit 1; }

  echo "[ContractWorktree] Archiving completed workflow before merge: $plan_file"
  bash ".ai/harness/scripts/archive-workflow.sh" --plan "$plan_file" --outcome Completed
}
...
finish_worktree() {
  ...
  bash ".ai/harness/scripts/verify-sprint.sh"
...
backfill_sprint_backlog() {
  ...
  [[ -f ".ai/harness/scripts/sprint-backlog.sh" ]] || return 0
  ...
  if bash .ai/harness/scripts/sprint-backlog.sh complete-task --sprint "$sprint_path" --task "$task_ref" --plan "$archived_plan"; then
...
cleanup_worktree() {
  ...
  if [[ -n "$worktree_path" ]]; then
    if [[ -n "$(git -C "$worktree_path" status --porcelain=v1 --untracked-files=all)" ]]; then
      ...
      echo "contract-worktree: pick/apply/commit useful changes first; scaffold-only discard belongs in .ai/harness/scripts/ship-worktrees.sh --cleanup-merged --discard-scaffold-only" >&2
      exit 1
    fi
  else
