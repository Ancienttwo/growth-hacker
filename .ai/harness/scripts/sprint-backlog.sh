...
usage() {
  cat <<'USAGE_EOF'
Usage:
  .ai/harness/scripts/sprint-backlog.sh init --slug <slug> [--title <title>]
  .ai/harness/scripts/sprint-backlog.sh status
  .ai/harness/scripts/sprint-backlog.sh next
  .ai/harness/scripts/sprint-backlog.sh start-task [--task <index|task>] [--execute] [--force] [--sprint <file>]
  .ai/harness/scripts/sprint-backlog.sh complete-task --task <index|task> [--plan <plan-file>] [--sprint <file>]
...
cmd_start_task() {
  ...
  [[ -f ".ai/harness/scripts/capture-plan.sh" ]] || { echo "sprint-backlog: .ai/harness/scripts/capture-plan.sh not found" >&2; exit 1; }
...
- [ ] Capture the approved \`\$think\` output with \`.ai/harness/scripts/capture-plan.sh --source waza-think --source-ref sprint:${sprint_file}#${target_task}\`
...
BODY_EOF
  ...
  if [[ "$target_mode" == "inline" ]]; then
    capture_output="$(REPO_HARNESS_DISABLE_CONTRACT_WORKTREE=1 bash .ai/harness/scripts/capture-plan.sh "${capture_args[@]}" 2>&1)" || {
      printf '%s\n' "$capture_output" >&2
      rm -f "$body_file"
      clear_in_flight "$target_task"
    ...
    capture_output="$(bash .ai/harness/scripts/capture-plan.sh "${capture_args[@]}" 2>&1)" || {
      printf '%s\n' "$capture_output" >&2
      rm -f "$body_file"
      clear_in_flight "$target_task"
      echo "sprint-backlog: capture-plan failed for task '$target_task'" >&2
