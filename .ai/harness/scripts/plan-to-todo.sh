...
usage() {
  cat <<'USAGE_EOF'
Usage: .ai/harness/scripts/plan-to-todo.sh --plan <plan-file>
USAGE_EOF
}
...
maybe_start_contract_worktree() {
  ...
  [[ -x ".ai/harness/scripts/contract-worktree.sh" ]] || return 0
  git rev-parse --is-inside-work-tree >/dev/null 2>&1 || return 0
  ! is_linked_worktree || return 0
  plan_requests_contract_worktree "$file" || return 0

  bash ".ai/harness/scripts/contract-worktree.sh" start --plan "$file"
  exit $?
}
