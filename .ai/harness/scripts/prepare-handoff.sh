...
if [[ -f ".ai/hooks/lib/workflow-state.sh" ]]; then
  ...
  if [[ "${REPO_HARNESS_SKIP_RESUME_REFRESH:-0}" != "1" && -f ".ai/harness/scripts/codex-handoff-resume.sh" ]]; then
    bash .ai/harness/scripts/codex-handoff-resume.sh --cwd "$(pwd -P)" --reason "${1:-manual}" >/dev/null
  fi
  exit 0
fi
