...
usage() {
  cat <<'USAGE_EOF'
Usage:
  .ai/harness/scripts/ship-worktrees.sh [--target <branch>] [--remote <name>] [--slug <slug>] [--ready] [--dry-run]
  .ai/harness/scripts/ship-worktrees.sh --local-merge [--target <branch>] [--slug <slug>] [--dry-run]
  .ai/harness/scripts/ship-worktrees.sh --cleanup-merged [--target <branch>] [--slug <slug>] [--discard-scaffold-only] [--dry-run]
...
require_finish_ready() {
  ...
  [[ -x ".ai/harness/scripts/contract-worktree.sh" ]] || fail ".ai/harness/scripts/contract-worktree.sh is missing or not executable"
  [[ -x ".ai/harness/scripts/verify-sprint.sh" ]] || fail ".ai/harness/scripts/verify-sprint.sh is missing or not executable"
  ...
  run_cmd bash ".ai/harness/scripts/verify-sprint.sh"
  if [[ "$DRY_RUN" -eq 1 ]]; then
    return 0
  fi
...
finish_contract_worktree() {
  ...
  if [[ "$merge_mode" == "local" ]]; then
    run_cmd bash ".ai/harness/scripts/contract-worktree.sh" finish --target "$TARGET_BRANCH"
  else
    run_cmd bash ".ai/harness/scripts/contract-worktree.sh" finish --no-merge --target "$TARGET_BRANCH"
  fi
}
...
- \`bash .ai/harness/scripts/verify-sprint.sh\` passed before \`contract-worktree.sh finish --no-merge\`.
...
ship_primary_pr() {
  ...
  while IFS=$'\t' read -r branch path; do
    ...
    (cd "$path" && bash ".ai/harness/scripts/ship-worktrees.sh" --target "$TARGET_BRANCH" --remote "$REMOTE_NAME" "${child_args[@]}")
    shipped=1
...
ship_primary_local_merge() {
  ...
  while IFS=$'\t' read -r branch path; do
    ...
    (cd "$path" && bash ".ai/harness/scripts/ship-worktrees.sh" --local-merge --target "$TARGET_BRANCH" "${child_args[@]}")
    run_cmd bash ".ai/harness/scripts/contract-worktree.sh" cleanup --slug "$slug" --target "$TARGET_BRANCH"
    shipped=1
  done < <(list_contract_worktrees "$BRANCH_PREFIX")
...
cleanup_merged() {
  ...
  while IFS=$'\t' read -r branch path; do
    ...
    if git merge-base --is-ancestor "$branch" "$TARGET_BRANCH" >/dev/null 2>&1; then
      ...
      if [[ "$DRY_RUN" -eq 1 ]]; then
        run_cmd bash ".ai/harness/scripts/contract-worktree.sh" cleanup --slug "$slug" --target "$TARGET_BRANCH" --dry-run
      else
        run_cmd bash ".ai/harness/scripts/contract-worktree.sh" cleanup --slug "$slug" --target "$TARGET_BRANCH"
      fi
      cleaned=1
