...
function usage(): never {
  console.error(
    [
      "Usage:",
      "  .ai/harness/scripts/architecture-event.ts json-get --key <key> [--json <json>]",
      "  .ai/harness/scripts/architecture-event.ts safe-token --value <value>",
      "  .ai/harness/scripts/architecture-event.ts derive-scope --block <functional-block> [--format lines|json]",
      "  .ai/harness/scripts/architecture-event.ts repo-path --repo <repo> --path <path>",
      "  .ai/harness/scripts/architecture-event.ts event-json --ts <ts> --file-path <path> ... [--pretty]",
      "  .ai/harness/scripts/architecture-event.ts upsert-request --request-file <path> --event-json <json>",
      "  .ai/harness/scripts/architecture-event.ts upsert-from-request --source-request <path> --request-file <path>",
      "  .ai/harness/scripts/architecture-event.ts reindex-requests --index-file <path> --requests-dir <dir> [--check]",
      "  .ai/harness/scripts/architecture-event.ts request-info --request-file <path>",
      "  .ai/harness/scripts/architecture-event.ts sync-context-map --context-map <path> --block <path> ...",
      "  .ai/harness/scripts/architecture-event.ts sync-contract-files --functional-block <path> --contract-agents <path> --contract-claude <path> ...",
    ].join("\n")
  );
  process.exit(2);
}
...
function renderRequiredFollowUp(event: ArchitectureEvent): string {
  ...
  return [
    ...
    `- If this starts or advances durable execution, run \`.ai/harness/scripts/workstream-sync.sh ensure --block "${functionalBlock}" --request "${requestFile}"\`.`,
    "- After the snapshot or diagram is produced, run `.ai/harness/scripts/context-contract-sync.sh sync-latest` so the local architecture contract block links to the latest artifacts.",
  ].join("\n");
}
