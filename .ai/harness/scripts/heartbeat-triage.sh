...
usage() {
  cat <<'USAGE_EOF'
Usage: .ai/harness/scripts/heartbeat-triage.sh [run] [--repo PATH] [--inbox PATH] [--run-id ID] [--source manual|scheduled] [--json]
...
helper_path() {
  ...
  if [[ -f ".ai/harness/scripts/$helper_name" ]]; then
  ...
  if [[ ! -f "$inbox_file" ]]; then
    {
      ...
      echo
      echo "<!-- managed by .ai/harness/scripts/heartbeat-triage.sh; append-only run log -->"
      echo
    } > "$inbox_file"
  fi
