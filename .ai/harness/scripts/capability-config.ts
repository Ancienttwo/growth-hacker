...
function usage(): never {
  console.error(
    [
      "Usage:",
      "  .ai/harness/scripts/capability-config.ts add --prefix <path> [options]",
      "",
      "Options:",
...
function validateRegistry(repo: string): void {
  runChecked(repo, process.execPath, [".ai/harness/scripts/capability-resolver.ts", "validate", "--repo", repo, "--format", "text"]);
}
...
function syncContracts(repo: string, capability: Capability): void {
  ...
  runChecked(repo, "bash", [".ai/harness/scripts/context-contract-sync.sh", "sync-event", "--json", JSON.stringify(event)]);
}
...
function createWorkstream(repo: string, capability: Capability): void {
  runChecked(repo, "bash", [".ai/harness/scripts/workstream-sync.sh", "ensure", "--block", capability.prefixes[0]]);
}
