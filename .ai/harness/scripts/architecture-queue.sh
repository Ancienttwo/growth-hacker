...
usage() {
  cat <<'USAGE_EOF'
Usage:
  .ai/harness/scripts/architecture-queue.sh record --file <path>
  .ai/harness/scripts/architecture-queue.sh status [--format text|json|summary] [--gate]
  .ai/harness/scripts/architecture-queue.sh reindex [--check] [--quiet]
  .ai/harness/scripts/architecture-queue.sh triage --before <YYYY-MM-DD>
  .ai/harness/scripts/architecture-queue.sh check
USAGE_EOF
}
...
architecture_event() {
  if command -v bun >/dev/null 2>&1 && [[ -f ".ai/harness/scripts/architecture-event.ts" ]]; then
    bun .ai/harness/scripts/architecture-event.ts "$@"
    return $?
  fi
  return 127
}
...
architecture_event_required() {
  ...
  if [[ ! -f ".ai/harness/scripts/architecture-event.ts" ]]; then
    echo "architecture-queue: missing .ai/harness/scripts/architecture-event.ts" >&2
    return 127
  fi
  return 0
}
...
selected_blocks() {
  if [[ -x ".ai/harness/scripts/select-agent-context-blocks.sh" ]]; then
    ".ai/harness/scripts/select-agent-context-blocks.sh" "$repo" 2>/dev/null || true
    return 0
  fi
...
status_command() {
  ...
  case "$mode" in
    ...
    advisory)
      ...
      ;;
    strict)
      if ! command -v bun >/dev/null 2>&1 || [[ ! -f ".ai/harness/scripts/architecture-event.ts" || ! -f ".ai/harness/scripts/capability-resolver.ts" ]]; then
...
record_command() {
  ...
  if ! command -v bun >/dev/null 2>&1 || [[ ! -f ".ai/harness/scripts/architecture-event.ts" ]]; then
    echo "[ArchitectureQueue] WARN: bun and .ai/harness/scripts/architecture-event.ts are required to record $rel_path; skipping advisory queue update"
    exit 0
  fi

  capability_match=""
  if [[ -f ".ai/harness/scripts/capability-resolver.ts" ]]; then
    resolver_stderr="$(mktemp)"
    if ! capability_match="$(bun .ai/harness/scripts/capability-resolver.ts match --path "$rel_path" --format json 2>"$resolver_stderr")"; then
  ...
  elif [[ ! -f ".ai/harness/scripts/capability-resolver.ts" ]]; then
...
triage_command() {
  ...
  while IFS= read -r request; do
    ...
    if [[ -x ".ai/harness/scripts/archive-architecture-request.sh" ]]; then
      bash .ai/harness/scripts/archive-architecture-request.sh \
        --request "$request" \
        --status superseded \
        --artifact "$card" \
