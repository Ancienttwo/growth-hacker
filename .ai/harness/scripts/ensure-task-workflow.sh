...
usage() {
  cat <<'USAGE_EOF'
Usage: .ai/harness/scripts/ensure-task-workflow.sh [--new-plan] [--slug <slug>] [--title <title>]
USAGE_EOF
}
...
ensure_current_status_snapshot() {
  mkdir -p tasks
  if [[ -x ".ai/harness/scripts/refresh-current-status.sh" ]]; then
    bash ".ai/harness/scripts/refresh-current-status.sh" --clear --write --reason "ensure-task-workflow" >/dev/null 2>&1 || true
    return 0
  fi
    ...
    cat > ".ai/harness/policy.json" <<'POLICY_EOF'
{
  ...
  "plan_capture": {
    ...
    "rule": "Codex Plan mode and Waza think planning should capture decision-complete plans into plans/plan-*.md; implementation approval then projects the active approved plan through .ai/harness/scripts/plan-to-todo.sh"
  },
  "planning": {
...
if [[ ! -f "docs/spec.md" ]]; then
  if [[ -x ".ai/harness/scripts/new-spec.sh" ]]; then
    bash ".ai/harness/scripts/new-spec.sh"
  fi
fi
...
if [[ -z "$slug" ]]; then
  ...
  echo "Create one with: bash .ai/harness/scripts/ensure-task-workflow.sh --slug <slug> --title <title>"
  exit 0
fi
...
if [[ -x ".ai/harness/scripts/new-plan.sh" ]]; then
  bash ".ai/harness/scripts/new-plan.sh" --slug "$slug" --title "$title"
else
  echo "Missing .ai/harness/scripts/new-plan.sh" >&2
  exit 1
fi
