...
if [[ -f "$registry_file" ]]; then
  if command -v bun >/dev/null 2>&1 && [[ -f "$repo/.ai/harness/scripts/capability-resolver.ts" ]]; then
    (cd "$repo" && bun .ai/harness/scripts/capability-resolver.ts list --format prefixes 2>/dev/null || true) | emit_existing_dirs
    exit 0
  fi
