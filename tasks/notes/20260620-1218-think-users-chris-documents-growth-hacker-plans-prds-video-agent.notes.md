...
- Live preflight: `curl -sS --max-time 3 http://127.0.0.1:8642/health` returned `{"status":"ok","platform":"hermes-agent"}`; `hermes auth status xai-oauth` returned `xai-oauth: logged in`.
- Smoke config used isolated temp workspace `/tmp/growth-hacker-growthctl-live-smoke.FBMDz6`, `growthRoot` under that temp directory, API bind port `18881`, `videoWorkflowScheduler: true`, `hermesApiBaseUrl: http://127.0.0.1:8642`, and `allowedHermesAgents: ["growth-agent"]`.
- `bun /Users/chris/Documents/growth-hacker/apps/server/src/index.ts` from the temp config directory: started successfully and reported `Growth Hacker dashboard API listening on http://127.0.0.1:18881`; `curl -sS http://127.0.0.1:18881/api/health` returned the temp `growthRoot`.
- `bun run growthctl -- --server http://127.0.0.1:18881 video project create --input @examples/video-agent/project.json`: passed; project `vprj_mqmkqwql_5492ea9fd3884b4f`, revision `1`, source checksum `8d2b3a9f500926b23481852d59c7eaef8b219959a897f4630119c2249b8b0b80`.
- `bun run growthctl -- --server http://127.0.0.1:18881 video workflow start vprj_mqmkqwql_5492ea9fd3884b4f --idempotency-key growthctl-live-hermes-smoke-v1 --provider xai-oauth --model grok-4.3 --max-attempts 1`: passed; run `vrun_mqmkr3or_13f752916ab84735`, status `queued`, requested provider `xai-oauth`, requested model `grok-4.3`, and unique `maxAttempts` value `[1]`.
- `bun run growthctl -- --server http://127.0.0.1:18881 workflow status vrun_mqmkr3or_13f752916ab84735`: final observed status `waiting_approval`, current step `preproduction_approval`, progress `88`; all stages through `storyboard_document` reached `succeeded`, and `preproduction_approval` was requested but not decided.
- Artifact evidence: successful run registered 20 runtime artifacts under `/tmp/growth-hacker-growthctl-live-smoke.FBMDz6/growth/video-projects/...`, including raw Agent outputs, validated JSON, canonical/provider prompts, render manifest, storyboard, CSVs, and `preproduction-package`.
- `bun run growthctl -- --server http://127.0.0.1:18881 video package export vprj_mqmkqwql_5492ea9fd3884b4f --revision 1`: passed with exit code 0; export run `vrun_mqmkr3or_13f752916ab84735`, package artifact `vart_mqmku4ew_a89faad855514e0f`, artifact count `15`, file count `15`, relative directory `vprj_mqmkqwql_5492ea9fd3884b4f/revision-1-vrun_mqmkr3or_13f752916ab84735-2026-06-20T16-34-11-164Z-eaa5e725`.
- `bun run growthctl -- --server http://127.0.0.1:18881 workflow events vrun_mqmkr3or_13f752916ab84735`: returned 56 events; final events included `preproduction-package`, transition to `waiting_approval`, and `approval.requested` with risk `local_write`.
- Export file check: `/tmp/growth-hacker-growthctl-live-smoke.FBMDz6/growth/video-exports/...` contains 15 files, matching the package export response.
- Server shutdown via `SIGINT` exited 0. Follow-up `curl -sS --max-time 2 http://127.0.0.1:18881/api/health || true` failed to connect, confirming the local listener was closed.
- `repo-harness run check-task-workflow --strict` initially failed on handoff freshness after the live smoke; `repo-harness adopt --repo /Users/chris/Documents/growth-hacker --compact --sync-codegraph --json` refreshed handoff, CodeGraph, and harness runtime state, and final `repo-harness run check-task-workflow --strict` passed with `[workflow] OK`.
- No preproduction approval was granted, no paid media render was started, no upload/public publish path was invoked, and the user's real `~/.growth` state was not touched.
